  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        {/* Vùng nội dung in */}
        <div id="print-area" className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="p-6">
            {/* Header in */}
            <div className="text-center mb-4">
              <p className="text-[11px] font-bold uppercase text-gray-700">Đảng Bộ Tỉnh Đồng Tháp</p>
              <p className="text-[13px] font-bold uppercase text-[#da251d] leading-tight mt-0.5">{orgName || 'Đảng ủy xã Tân Dương'}</p>
              <h1 className="text-xl font-bold uppercase text-gray-800 tracking-tight mt-3">Lịch công tác tuần</h1>
                ...
                <tbody>
                  {weekDays.map((day) => {
                    ...
                    return daySchedules.map((s, index) => (
                      <tr key={s.id} className={`${isTodayActive ? "bg-yellow-50/50" : ""} hover:bg-gray-50 transition-colors`} style={{pageBreakInside: 'avoid'}}>
                        {index === 0 && (
                          <td 
